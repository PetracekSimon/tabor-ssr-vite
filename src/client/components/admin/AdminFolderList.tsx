        ...
        <>
            ...
            <nav className="flex mb-4" aria-label="Breadcrumb">
                <ol className="inline-flex items-center space-x-1 md:space-x-3">
                    <li className="inline-flex items-center">
                        <a href="#" className="inline-flex items-center text-sm font-medium text-gray-700 hover:text-primary-600" onClick={() => props.handleFolder(true, false, 0)}>
                            <HomeIcon />
                    ...
                    {props.currentPath.map((folder, index) => (
                        <li key={index}>
                            <div className="flex items-center">
                                ...
                                <a href="#" className="ml-1 text-sm font-medium text-gray-700 hover:text-primary-600 md:ml-2" onClick={() => props.handleFolder(false, true, index + 1, folder)}>
                                    {folder.name}
                                </a>
                            </div>
                        </li>
            ...
            <ul className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                {props.folders.map((folder) => (
                    <li key={folder._id} className="bg-white border border-gray-200 rounded-lg p-4 flex flex-col items-center justify-center hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => props.handleFolder(false, false, 0, { name: folder.name, code: folder.code, _id: folder._id })}>
                        <FolderIcon />
                        <span className="mt-2 text-sm font-medium text-gray-900 truncate w-full text-center">{folder.name}</span>
                    </li>
                ))}
                <li className="bg-white border border-gray-200 rounded-lg p-4 flex flex-col items-center justify-center hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => props.setCreateFolderModal(true)}>
                    <FolderAddIcon />
                    <span className="mt-2 text-sm font-medium text-gray-900 truncate w-full text-center">Nová složka</span>
                </li>
            </ul>
